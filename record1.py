...
def get_voice(usrname):

    CHUNK = 1024
    FORMAT = pyaudio.paInt16
    CHANNELS = 2
    RATE = 11025
    WAVE_OUTPUT_FILENAME = usrname + ".wav"
    RECORDING = False

    def record_thread(fileName, stream, p):
        global RECORDING
        print('recording')
        waveFile = wave.open('./voice_database/'+fileName, 'wb')
    ...
    layout = QVBoxLayout()
    btn = QPushButton()
    g = record_generator(WAVE_OUTPUT_FILENAME, btn)
    g.next()
